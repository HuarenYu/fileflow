...
- `preview_missing_file_returns_error`：不存在的路径返回 `Err`
...
`invoke` 触发的验证放在 hook 测试中：

- 调用 `search` 函数后，`invoke("search_files", ...)` 被调用
- 空查询不触发 `invoke`
...
- `insert_then_query_vector`：插入含已知向量的 chunk，向量查询返回正确记录
